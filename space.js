...
var gameSpeed = 0.2;
var canRestart = true;
var bullets, bullet, spaceObjects, ave;
var bigFlame = true;
var drawAsteroid, drawEndingScreen;
var starsX, starsY, keys, moving = false;
...
setup = function() {
  ...
  bullet = function(x, y) {
    this.x = x;
    this.y = y;
    xDist = Math.abs(mouseX - x);
    yDist = Math.abs(mouseY - y);
    angle = Math.tan(yDist/xDist);
    if(mouseX -x < 0)
        sign = -1;
    else
        sign = 1
    this.xvel = gameSpeed * Math.cos(angle);
    if(mouseY -y < 0)
        sign = -1;
    else
        sign = 1
    this.yvel = gameSpeed * Math.sin(angle);
...
drawEndingScreen = function() {
  ...
  text(
    ...
    textSize(0.05 * width);
  text("Score " + counter, width / 2, Math.round(0.4 * height));
  //THE FOLLOWING CODE WORKS WITH THE DATABASE, IFF RUNNING INSIDE FLASK APP
  /*
...
var updateObjects = function() {
  for (var i = 0; i < spaceObjects.length; i++) {
    ...
    if(hardMode){
        spaceObjects[i].x += spaceObjects[i].xspeed;
    }
    ...
    if (spaceObjects[i].x > width * 1.15) {
        spaceObjects[i].x = -50;
    }
    if (spaceObjects[i].x < -50) {
        spaceObjects[i].x = 1.1 * width;
    }
    var d = dist(
      xPos + aWidth / 2,
      yPos - aHeight / 2,
